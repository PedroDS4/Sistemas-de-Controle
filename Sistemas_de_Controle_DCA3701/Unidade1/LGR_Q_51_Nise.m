% Definindo o numerador e denominador da função de Transferência G(s)H(s)
num = [1 -5];          % s + 2
den = [1 167/3 860/3 500/3];      % s^2 - 4s + 13
...
fprintf('Polos (p):          %s\n', mat2str(p, 3));
fprintf('Zeros (z):          %s\n', mat2str(z, 3));
fprintf('Lugares Separados (Ls):  %.4f\n', LS);

fprintf('------------------- ASSÍNTOTAS ---------------------------\n');
fprintf('Loc Assíntota (sigma_A): %.3f\n', sigma_A);
fprintf('Ângulos das Assíntotas (phi_A):     %s°\n', mat2str(phi_A, 3));

fprintf('------------------- PONTOS CRÍTICOS ----------------------\n');
fprintf('Pontos de Saída do Eixo Real (sigma_out): %s\n', mat2str(sigma_out, 3));
fprintf('Cruzamento do eixo imaginario Jw (w_c):        %s rad/s\n', mat2str(w_cross, 3));

fprintf('------------------- ÂNGULOS DE PARTIDA/CHEGADA -----------\n');
fprintf('Partida (phi_in):    %s°\n', mat2str(phi_in, 3));
fprintf('Chegada (theta_out): %s°\n', mat2str(theta_out, 3));

fprintf('========================Verificação de Ponto================================\n\n');
